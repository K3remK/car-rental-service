...
create table reservation (
                             ...
                             drop_off_date_and_time timestamp(6),
                             pick_up_date_and_time timestamp(6),
                             status varchar(30) not null,
                             car_id uuid,
                             customer_ssn varchar(255),
                             drop_off_location_code bigint,
                             pick_up_location_code bigint,
                             primary key (reservation_number)
);

create table ref_reservation_extras (
                                        reservation_number varchar(255) not null,
...
alter table if exists customer
    add constraint UK_license unique (driving_license_number);

alter table if exists customer
    add constraint UK_phone unique (phone_number);

alter table if exists ref_reservation_extras
    add constraint UK_res_extras unique (reservation_number, extra_service_id);

alter table if exists car
    add constraint UK_licenseplate unique (license_plate_number);

alter table if exists car
    add constraint FK_car_location
        foreign key (location_code) references location;

alter table if exists ref_reservation_extras
    add constraint FK_extras_service
        foreign key (extra_service_id) references extra_service;

alter table if exists ref_reservation_extras
    add constraint FK_extras_res
        foreign key (reservation_number) references reservation;

alter table if exists reservation
    add constraint FK_res_car
...
alter table if exists reservation
    add constraint FK_res_dropoff
        foreign key (drop_off_location_code) references location;

alter table if exists reservation
    add constraint FK_res_pickup
        foreign key (pick_up_location_code) references location;
